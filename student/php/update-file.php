...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_FILES['file']) && $_FILES['file']['error'] === 0) {
            ...
            mysqli_stmt_bind_param($stmt, "ssssi", $fileName, $fileDestination, $fileType, $fileStatus, $fileId);
        } else {
            // Update only file type
            $query = "UPDATE tblfile SET fileType = ?, status = ? WHERE id = ?";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, "ssi", $fileType, $fileStatus, $fileId);
        }
