...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['password']) && !empty($_POST['password'])) {
        ...
        $stmt = $con->prepare("UPDATE tblstudents SET firstName=?, lastName=?, email=?, status=?, password=? WHERE id=?");
        $stmt->bind_param("sssssi", $firstName, $lastName, $email, $status, $hashedPassword, $id);
    } else {
        $stmt = $con->prepare("UPDATE tblstudents SET firstName=?, lastName=?, email=?, status=? WHERE id=?");
        $stmt->bind_param("ssssi", $firstName, $lastName, $email, $status, $id);
    }

    // Execute the statement
    if ($stmt->execute()) {
        echo "<script>alert('Student updated successfully'); window.location.href = '../manage-student.php';</script>";
    } else {
        echo "<script>alert('Failed to update student'); window.location.href = '../manage-student.php';</script>";
    }
